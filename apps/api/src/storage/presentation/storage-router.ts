...
import { createSignedUrlUseCase } from "../app/create-signed-url-use-case";
...
export const storageRouter = honoRouter((ctx) => {
  const router = new Hono()
    ...
    .post(
      "/signed-url",
      describeRoute({ description: "Generate Signed URL for files" }),
      zValidatorWrapper("json", CreateSigneUrlInputSchema),
      validator("json", CreateSigneUrlInputSchema),
      async (c) => {
        const body = c.req.valid("json");
        const user = getUserInContext(c);
        const result = await createSignedUrlUseCase(ctx, {
          body,
          user: user,
        });
        return handleResultResponse(c, result, { oppSysContext: ctx });
