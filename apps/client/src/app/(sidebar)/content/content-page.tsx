...
export default function ContentPage() {
  ...
  const handleUserDecision = async (contentId: string, isApproved: boolean) => {
    if (!user) {
  ...
  const pageCount =
    pagination?.pages ||
    Math.max(1, Math.ceil((pagination?.total || 0) / itemsPerPage));
  const paginatedContent = filteredAndSortedContent; // server already paginates
